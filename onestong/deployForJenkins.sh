...
destAbsPath=/Users/ost/Documents/projects/yst-crm
configFolder=conf
##############################################################
...
function stopServer(){
  ...
  # ps -efx|grep "$projectName" | awk "{print $2}" | xargs kill -9   
  lsof -P | grep ":$serverPort" | awk "{print $2}" | xargs kill -9  
  echo "lsof -P | grep ":$serverPort" | awk "{print $2}" | xargs kill -9 "
   echo " success "
}
...
function run(){
   ...
   nohup nice java -jar $destFile --spring.profiles.active=$env >/dev/null 2>&1 &

   echo "COMMAND: nohup nice java -jar $destFile  $properties --spring.profiles.active=$env >/dev/null 2>&1 &"
