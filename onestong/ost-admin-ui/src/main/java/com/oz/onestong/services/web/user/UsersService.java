package com.oz.onestong.services.web.user;
...
import com.oz.onestong.controller.base.Action.OST_APP_PERM;
import com.oz.onestong.controller.base.Action.OST_OST_PERM;
...
import com.oz.onestong.model.department.DepartmentExample;
...
import com.oz.onestong.model.user.UserExample.Criteria;
...
public class UsersService {
	...
	public boolean addUser(User user, Account account, Role role) throws NoSuchAlgorithmException {
		...
		// 设置部门名称
		user.setDepartmentName(this.departmentMapper.selectByPrimaryKey(user.getDeptId()).getName());

		// 设置公司名称
		user.setCompanyName(this.domainMapper.selectByPrimaryKey(user.getDomainId()).getDomainName());
		// 设置角色
		...
		String contentString = sms.GetSendMsg(user.getRealname(), user.getPhoneNum(), orginalPassword);
		...
		if (user == null) {
			...
			if (StringUtils.isNotBlank(user.getDeviceId()) && !(user.getDeviceId().equals(deviceId))) {
				...
				if (!(account.getLoginPassword().equals(Md5Util.md5(password.toString().trim())))) {
					...
					this.userMapper.updateByExampleSelective(user, ue);
					// 更新token
					String token = com.oz.onestong.tools.UniqueTokenGenerator.generateUniqueToken();
					ae.clear();
...
//	@Transactional(rollbackFor = { Exception.class }, propagation = Propagation.REQUIRED)
//	public String updateUserPic(User user, MultipartFile headPic) throws IOException {
//		String thumPic = "";
//		String fileName = "";
...
//			compress.compressPic(Constants.FILE_SAVE_DIR, Constants.FILE_SAVE_DIR, fileName, thumPic, 44 * 3, 44 * 3,
//					true);
//			// 小图 文件名
//			r.setName1(thumPic);
