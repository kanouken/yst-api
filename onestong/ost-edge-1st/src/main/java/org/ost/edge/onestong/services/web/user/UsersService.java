...
public class UsersService {
	...
	@Transactional(readOnly = true)
	public List<User> findAllUsers(User user, RowBounds rb,
	...
	private boolean hasDeptDirectorSymbolOrNot(List<Map<String, Object>> perms) {
		...
		for (Map<String, Object> tmp : perms) {
			...
			if (authTag.equalsIgnoreCase(Action.OST_OST_PERM.unBindUser
					.toString())) {
	 ...
	 */
	@Transactional(rollbackFor = { Exception.class }, propagation = Propagation.REQUIRED)
	public boolean addUser(User user, Account account, Role role)
			throws NoSuchAlgorithmException {

		List<Map<String, Object>> perms = this.authorityService
				.findPermsByRoleAndType(role, Constants.MOUDLE_ADMIN);
...
//		}
	...
	private void fillByUserBlow(Department dept, List<User> users,Boolean fetchDirectorOnly) {
		...
		ue.createCriteria().andDeptIdEqualTo(dept.getDeptId())
		...
		if (dept.getIsLeaf().equals(Constants.IS_LEAF)) {
			...
			de.createCriteria().andValidEqualTo(Constants.DATA_VALID)
					.andParentIdEqualTo(dept.getDeptId());
			List<Department> childrenDepts = departmentMapper
					.selectByExample(de);
	...
	public List<User> findAllUsersDomain(Integer domainId) {
		UserExample ue = new UserExample();
		ue.createCriteria().andDomainIdEqualTo(domainId)
				.andValidEqualTo(Constants.DATA_VALID)
	...
	@Transactional(readOnly = true)
	public List<User> findUserScopes(final User user,
			List<Map<String, Object>> perms) {
		UserExample ue = new UserExample();
		...
		} else if (AuthorityService.isPermExits(OST_APP_PERM.self_below_events,
				perms)) {
	...
	public User findoneByEmail(String email) {
		UserExample ue = new UserExample();
		ue.createCriteria().andValidEqualTo(Constants.DATA_VALID)
				.andStateEqualTo(Constants.USER_NORMAL).andEmailEqualTo(email);
	 ...
	 */
	@Transactional(rollbackFor = { Exception.class }, propagation = Propagation.REQUIRED)
	public OperateResult login(Object email, Object password, Object deviceId,
			Object clientId) throws NoSuchAlgorithmException {
		// 根据 邮箱 查找用户
		...
		if (user == null) {
			...
			if (StringUtils.isNotBlank(user.getDeviceId())
				...
				if (StringUtils.isBlank(user.getDeviceId())) {
					...
					if (null != this.findoneByDeviceId(deviceId.toString()
							.trim())) {
				...
				ae.createCriteria().andValidEqualTo(Constants.DATA_VALID)
						.andStateEqualTo(Constants.USER_NORMAL)
				...
				if (!(account.getLoginPassword().equals(Md5Util.md5(password
						.toString().trim())))) {
					...
					// 拉取 app权限
					Role r = this.usersRolesMapper.selectRolesByUser(
							user.getUserId()).get(0);

					List<Map<String, Object>> perms = this.rolesMoudlesPermsMapper
							.selectPermsByRoleAndType(r.getRoleId(),
									Constants.MOUDLE_APP);
					// 没有 app 权限 不能登陆
					...
					// TODO 判断 账户是否在有限期内
					if (!this.domainOrderService.isValidActive(user
							.getDomainId())) {
					...
					String token = UniqueTokenGenerator
							.generateUniqueToken();
					ae.clear();
					ae.createCriteria().andAccountIdEqualTo(
					...
					Department dept = this.departmentMapper
							.selectByPrimaryKey(user.getDeptId());
					...
					iie.createCriteria().andDepartmentIdEqualTo(
							user.getDeptId());
					List<IbeaconInfo> ibeacons = this.ibeaconInfoMapper
							.selectByExample(iie);
					...
					// 初始化我点赞的事件
					resultMap.put("myLikedEvents", this.likeMapper
							.selectMyLikedEventIds(user.getUserId()));
	...
	@Transactional(rollbackFor = { Exception.class }, propagation = Propagation.REQUIRED)
	public void updateUser(User user, Account account, Role role)
			throws NoSuchAlgorithmException {
		List<Map<String, Object>> perms = this.authorityService
				.findPermsByRoleAndType(role, Constants.MOUDLE_ADMIN);
		...
		String newDeptName = this.departmentMapper.selectByPrimaryKey(
				user.getDeptId()).getName();
	...
	@Transactional(rollbackFor = { Exception.class }, propagation = Propagation.REQUIRED)
	public String updateUserPic(User user, MultipartFile headPic)
			throws Exception {
		String thumPic = "";
		String fileName = "";
		if (headPic.getSize() > 0 && !headPic.isEmpty()) {
			...
			r.setName(fileName);
			// 图片需要压缩 44*3 44*
			thumPic = new StringBuilder(fileName).insert(
