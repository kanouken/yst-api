...
public interface VisitEventEntityMapper {
	...
	@Mappings({ @Mapping(source = "vo.customer.id", target = "customerId"), @Mapping(constant = "1", target = "valid"),

			@Mapping(source = "vo.customer.name", target = "customerName"),

			@Mapping(source = "vo.contacts.id", target = "contactsId"),
			@Mapping(source = "vo.contacts.name", target = "contactsName"),
	...
	VisitEvents visitEventCreateVoToVisitEvents(VisitEventCreateVo vo);

	@Mappings({ @Mapping(source = "contactTime", dateFormat = "yyyy-MM-dd HH:mm", target = "contactTime"),
			@Mapping(source = "veId", target = "id") })
	VisitEventDetailVo visitEventsToVisitEventDetailVo(VisitEvents vEvents);
